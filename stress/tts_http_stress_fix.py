...
class Test(unittest.TestCase):
    ...
    StressDataDir = "../Data/StressData_fix/Data/"
    StressUyghurDataDir = "../Data/StressData_fix/UyghurData/"
    Stresswangyu = "../Data/StressData_fix/UyghurData/"
    StressDataList = os.listdir(StressDataDir)
    StressUyghurList = os.listdir(StressUyghurDataDir)
    ...
    def text_tts_process_main(self, thread_name, thread_index):
        ...
        while (True):
            ...
            if ((datetime.datetime.today()-start_time)>self.CYCLE_TIME):
                ...
                StressUyghurList = self.StressUyghurDataDir + random.choice(self.StressUyghurList)
                file = open(TestDataPath, "rb").read()
                fileUyghur = open(StressUyghurList, "rb").read()
                ttscfg = self.GetRandomConfig()
                if random.randint(0, 1) == 1:
                    is_Ex = True
                else:
                    is_Ex = False   
                if ttscfg.find("uyghur") != -1 :
                    print ttscfg.find("usy_uyghur_common")
                    file = fileUyghur
                    print "file change"
                try:
                    ...
                    self.test_errorlog.write('【ERROR】\t【info】:' + str(e) + '\n')
                    self.mutex.release()
                    continue
                self.mutex.acquire() 
